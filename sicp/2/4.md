...
::: details INFO
译者：[Joseph_L](https://github.com/Joeljt)、[Congwen-Wang](https://github.com/Congwen-Wang)、[clcs](https://github.com/clcs)

来源：[2.4 Mutable Data](https://www.composingprograms.com/pages/24-mutable-data.html)
:::

我们已经看到，抽象对于帮助我们应对大型系统的复杂性至关重要。有效的程序设计还需要组织原则，以指导我们制定程序的整体设计。特别是，我们需要策略来帮助我们将大型系统构建为**模块化**（modular）的，也就是说，它们可以自然地划分为连贯的部分，而这些部分可以被单独开发和维护。

创建模块化程序的一种强大技术是引入**随时间改变状态的数据**。通过这种方式，单个数据对象可以代表某种独立于程序其余部分而演变的事物。变化对象的行为可能受其历史影响，就像现实世界中的实体一样。给数据添加**状态**是面向对象编程（object-oriented programming）范式的核心要素。
...
名称 `date` 绑定到一个类（class）。正如我们所见，一个类代表一种值的类型。具体的日期被称为该类的实例（instances）。可以通过调用类并传入描述该实例的参数来构造（constructed）实例。

```python
>>> tues = date(2014, 5, 13)
```

虽然 `tues` 是由基础数字构造的，但它的行为像一个日期。例如，用另一个日期减去它会得到一个时间差，我们可以将其打印出来。
...
对象有**属性**（attributes）的概念，可以理解为该对象中某个值的名字。和其它许多编程语言一样，我们在 Python 中使用点语法来访问一个对象中的某个属性。

```python
>>> <expression> . <name>
```

在上面的代码中，`<expression>` 表示一个对象，`<name>` 是该对象的一个属性名称。
...
对象还有**方法**（methods）的概念，其实也是属性，只不过该属性的值是函数。对象“知道”如何执行这些方法。在实现上，方法是根据它们的参数以及它们所属的对象来计算结果的函数。例如，`tues` 的 `strftime` 方法（一个经典的函数名，意为“string format of time”时间字符串格式）接受一个参数，指定如何显示日期（例如，`%A` 意味着星期几应该全拼出来）。
...
要计算 `strftime` 的返回值需要两个输入：期望展示的时间格式，以及 `tues` 中包含的日期信息。这个方法内部已经有了处理日期相关的逻辑，并且能够返回我们期望的结果。我们从来没有说过 2014 年 5 月 13 日是星期二，但是日期这个类本身就有这种能力，它能够知道一个特定的日期应该是星期几。通过将行为和信息捆绑在一起，这个 Python 对象为我们提供了一个令人信服、自包含的日期抽象。

日期是对象，但数字、字符串、列表和范围也都是对象。它们代表值，但行为方式也符合它们所代表的值。它们也有属性和方法。例如，字符串有一系列便于文本处理的方法。
...
事实上，Python 中**所有**的值都是对象。也就是说，所有的值都有行为和属性。它们的行为表现就像它们所代表的值一样。

## 2.4.2 序列对象

像数字这样的基本数据类型的实例是**不可变**（immutable）的。这些值本身在程序执行过程中不能改变。另一方面，列表是**可变**的（mutable）。
...
**数据共享和身份**（Sharing and Identity）：正是由于我们没有在操作数据时创建新的列表，而是直接操作的源数据，这就导致变量 `chinese` 也被改变了，因为它和变量 `suits` 绑定的是同一个列表！
...
字典的 `key` 一般都是字符串，因为我们习惯用字符串来表示某个事物的名称。下面这个字典字面量表示了一组罗马数字：
...
> 译注：Python 3.7 及以上版本的字典顺序会确保为插入顺序，此行为是自 3.6 版开始的 CPython 实现细节，字典会保留插入时的顺序，对键的更新也不会影响顺序，删除后再次添加的键将被插入到末尾。
