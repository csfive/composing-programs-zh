...
对象 (objects) 将数据值与行为结合起来。对象表示信息，也表示它所代表事物的行为。对象如何与其他对象交互的逻辑与编码对象值的信息捆绑在一起。当一个对象被打印出来时，它知道如何在文本中拼写自己。如果一个对象由多个部分组成，它知道如何按需显示这些部分。对象既是信息又是过程，它们捆绑在一起以表示复杂事物的属性、交互和行为。
