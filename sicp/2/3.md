...
列表（list）是一个可以有任意长度的序列。列表有大量的内置行为，以及用于表达这些行为的特定语法。我们已经见过列表文字（list literals），它的计算结果是一个 `list` 实例，以及一个计算结果为列表中元素值的元素选择表达式。`list` 内置的 `len` 函数返回序列的长度。如下，`digits` 是一个包含四个元素的列表，索引为 3 的元素是 8。
...
1. 评估头部（header）中的 `<expression>`，它必须产生一个可迭代（iterable）的值（译者注：可迭代的详细概念可见 *4.2 隐式序列*）
2. 对该可迭代值中的每个元素，按顺序：
...
将 `range` 的计算结果传入 `list` 构造函数，可以构造出一个和 `range` 具有相同范围的列表，从而简单的检查元素值。
...
为了计算列表推导式，Python 首先评估 `<sequence expression>`，它必须返回一个可迭代值。然后将每个元素值按顺序绑定到 `<name>`，再执行 `<filter expression>`，如果结果为真值，则计算 `<map expression>`，`<map expression>` 的结果将被收集到结果列表中。
...
字符串文字（string literals）可以表示任意文本，使用时将内容用单引号或双引号括起来。
...
**多行文字（Multiline Literals）**：字符串可以不限于一行。跨越多行的字符串文字可以用三重引号括起，我们已经在文档字符串中广泛使用了这种三重引号。
