...
使用列表作为其他列表中元素的能力为编程语言提供了一种新的组合方式。这种能力称为数据类型的闭包属性 (closure property) 。一般来说，如果使用某种数据值组合方法得到的结果也可以用同一方法进行组合，则该方法具有闭包属性。闭合是任何组合方式的关键，因为它使我们可以创建层次结构——由“部分”组成的结构，“部分”其本身也由部分组成。
