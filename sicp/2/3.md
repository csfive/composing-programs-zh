...
一个 list 值是一个可以有任意长度的序列。 list 有大量的内置行为，以及表达这些行为的特定语法。我们已经见过列表字面量，它的计算结果是一个 list 实例；以及一个元素选择表达式，该表达式的计算结果是列表中的一个值。内置的 `len` 函数返回序列的长度。如下，digits 是一个包含四个元素的列表。索引为 3 的元素是 8。
