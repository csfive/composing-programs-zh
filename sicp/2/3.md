...
**序列（sequence）**是一组有顺序的值的集合，是计算机科学中强大且基础的抽象。它不是某个特定内置类型或表示，而是一类共享行为的集合：种类很多，行为一致，尤其：
...
列表（list）可以有任意长度，内置了大量行为与语法。我们已经见过列表字面量和求出元素值的元素选择表达式。`len` 返回序列长度。下例中 `digits` 有四个元素，索引 3 是 8。
...
多个列表可以相加，列表也能乘以整数。对序列来说，加法与乘法作用在序列自身的拼接与复制上。`operator.add`（`+`）会串联列表，`operator.mul`（`*`）接收列表和整数 k，返回元素重复 k 次的新列表。
...
任何值都能放入列表，包括另一个列表。嵌套列表可多次元素选择以取得更深层元素。
...
我们常常需要依次遍历序列并按元素做计算，所以 Python 提供了额外的控制语句：`for` 循环。

考虑统计某个值在序列中出现多少次，可以用 `while` 写一个函数。
...
执行过程中用到了可迭代值。列表是序列，序列是可迭代的，元素按顺序被遍历。Python 还有其他可迭代类型，但此处聚焦序列；“iterable” 的一般定义在第 4 章讲迭代器时给出。

一个重要结果是：执行 `for` 语句后，`<name>` 会绑定到序列的最后一个元素，因此 `for` 也能更新环境。
...
这种把多个名称绑定到固定长度序列多个值的模式称为**序列解包（sequence unpacking）**，类似赋值语句中的 `x, y = 3, 4.5`。

**范围（Ranges）**：`range` 是表示整数范围的内置序列，用起始值和结束值加一创建（也可传步长）。
