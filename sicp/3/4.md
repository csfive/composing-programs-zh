...
我们将首先定义一种语言的解释器，它是 Scheme 的一个有限子集，称为 Calculator。然后，我们将为 Scheme 整体开发一个简略的解释器。我们创建的解释器将是完整的，因为它允许我们用 Scheme 编写完全通用的程序。为此，它将实现我们在第1章为 Python 程序引入的相同的运算模型。
