...
来源：[4.4 Logic Programming](http://composingprograms.com/pages/44-logic-programming.html)

对应：无
:::

在本节中，我们将介绍一种专门为本节内容设计的声明式查询语言 logic。这是一种基于 [Prolog](http://en.wikipedia.org/wiki/Prolog) 和 [计算机程序结构与解释](http://mitpress.mit.edu/sicp/full-text/book/book-Z-H-29.html#%_sec_4.4.1) 的声明式语言。[logic](http://composingprograms.com/examples/logic/logic..html) 解释器是基于前面章节的模式项目的一个完全实现。其中，数据记录被表示为模式列表，查询语句被表示为模式值。
