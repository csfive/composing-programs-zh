...
在這個例子中，當創建 `range` 實例時，並沒有存儲此範圍內的 999,990,000 整數。反之，`range` 對像將第一個元素 10,000 添加到索引 45,006,230 來得出元素 45,016,230
。按需要來計算值，而不是從現有的表示中去檢索他們，這是惰性計算（Lazy computation）的一個範例。在計算機科學中，惰性計算指任何延遲計算，直到需要該值的程序。
...
5
```
在 Python 中表示沒有更多可用值的方式是在調用 `next` 時引發 `StopIteration` 錯誤。可以使用 `try` 語句來處理此錯誤。
...
`map` 函數是惰性的：調用它不會執行計算其結果元素所需的計算。
...
StopIteration
```
生成器在第一次調用 `__next__` 之前，不會執行任何在生成器函數內的語句。生成器會觸發 `StopIteration` 錯誤當它遍歷完生成器函數中定義的範圍後。
...
迭代器將引發 `StopIteration` 錯誤。許多應用程序需要多次對元素進行迭代。例如，我們必須多次遍歷列表，以列舉所有元素對。
...
因此，對 `__next__` 的多次調用會返回底層序列的連續元素。當在調用 `__next__` 時引發 `StopIteration` 錯誤，表示已經達到了序列的末端。
...
例如： `LetterIter` 實例只能迭代一次。直到 `__next__()` 方法觸發 `StopIteration` 錯誤。

通常，迭代器不會被重置；取而代之，會創建一個新實例來開始新的迭代。

迭代器也允許我們通過實現永遠不會觸發 `StopIteration` 錯誤的 `__next__` 方法來表示無限級數。
