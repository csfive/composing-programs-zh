...
::: details INFO
译者：[DuKle3](https://github.com/DuKle3)、[clcs](https://github.com/clcs)

来源：[4.2 Implicit Sequences](https://www.composingprograms.com/pages/42-implicit-sequences.html)

对应：HW04、Disc05、Lab07
:::

序列（sequence）的表示不一定需要将每个元素显式地存储在计算机内存中。也就是说，我们可以构造一个对象，它能够提供对某个序列数据集中所有元素的访问，而无需提前计算出每个元素的值。相反，我们可以在需要时才计算这些元素。

第 2 章中介绍的 `range` 容器类型就体现了这一思想。一个 `range` 表示一串连续的、有范围的整数序列。然而，该序列的每个元素并不是显式地存储在内存中的。相反，当从 range 中请求一个元素时，它才被计算出来。因此，我们能够表示非常大的整数范围，而无需使用大块内存。`range` 对象中只存储了序列的端点值。
...
在这个例子中，`range` 实例在被构造时并没有将这 999,990,000 个整数全部存储起来。相反，`range` 对象将起始元素 10,000 加上索引 45,006,230，从而得到元素 45,016,230。在需要时才计算值，而不是从已有的表示中检索值，这就是**惰性求值**的一个实例。在计算机科学中，惰性求值（lazy computation）泛指任何将值的计算推迟到真正需要时再进行的程序。

## 4.2.1 迭代器

Python 以及许多其他编程语言提供了一种统一的方式来依次处理容器值中的元素，称为**迭代器**。迭代器（iterators）是一个对象，它能够逐个地提供对值的顺序访问。

迭代器抽象包含两个组成部分：一个用于获取序列中下一个元素的机制，和一个用于标识序列已经到达末尾、没有更多元素了的信号机制。
...
在 Python 中表示没有更多可用值的方式是在调用 `next` 时抛出 `StopIteration` 异常，可以使用 `try` 语句来处理此错误。
...
迭代器会维护一个内部状态来表示它在序列中的位置。每次调用 `next`，该位置就会向前推进。两个独立的迭代器可以跟踪同一序列中的两个不同位置。但如果两个名称指向同一个迭代器，它们将共享相同的位置，因为它们共享同一个值。
...
推进第二个迭代器不会影响第一个迭代器。由于第一个迭代器最后返回的值是 4，下一次返回 5。而第二个迭代器的位置会下一次返回 7。
...
> 译注：现在 Python 3.6+ 版本，字典的顺序是键值对（key-value pair）加入字典时的顺序；在 Python 3.5 和之前的版本，字典是无顺序的
...
> 译注：这里没有 `LetterIter` 的实现，就把他当作可迭代的值。
...
> 译注：在 Python 3.3 中，加入了 `yield from` 语句，可以在 [Lecture](https://www.youtube.com/watch?v=eIBV4fsxnjE&list=PL6BsET-8jgYXMKOdcoi0Hy_Gn4fuY_XzL&index=7) 或是 [Python 3.3 更新文档](https://docs.python.org/3/whatsnew/3.3.html) 取得更多相关资讯。
