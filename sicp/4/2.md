...
任何可以产生迭代器的值都称为可迭代值（iterable vallue）。
在 Python 中，可迭代值是任何可以传递给内置 `iter` 函数的值。
可迭代对象包括：
...
[('a', 'a'), ('a', 'b'), ('a', 'c'), ('a', 'd'), ('b', 'a') ]
```
> 译者注：在Python 3.3 中，加入了`yield from` 语句，可以在[Lecture](https://www.youtube.com/watch?v=eIBV4fsxnjE&list=PL6BsET-8jgYXMKOdcoi0Hy_Gn4fuY_XzL&index=7) 或是[Python 3.3 更新文档](https://docs.python.org/3/whatsnew/3.3.html) 取得更多相关资讯。
...
通过将素（质）数流转换为列表，它不会被「用完」。即流的前缀转换为列表后， `prime_numbers` 的第一个元素仍然是2。
